...
class MPStructure(Structure):
    def __init__(self, mpParser: MPParser = None, lattice: Lattice = None,
                 composition: List = None,
                 sites: List[Site] = None,
                 isinit: bool = False):
        self.mpParser = mpParser
        if self.mpParser is not None and not self.mpParser.hasSetup:
            self.mpParser = None
        self.isinit = isinit
        if mpParser is not None:
            composition = mpParser.getComposition()
            if isinit:
                lattice = mpParser.lattice_s
                sites = mpParser.sites_s
            else:
                lattice = mpParser.lattice_e
                sites = mpParser.sites_e
    ...
    def setup(self):
        ...
        if self.mpParser is not None:
            self.generalFormula = self.mpParser.generalFormula
            self.prototype = self.mpParser.getProtoType()
            if self.isinit:
                self.cellStress = self.mpParser.cellStress_s
                self.volume = self.mpParser.volume_s
                self.spaceGroup, self.pointGroup = self.getSpacePointGroup()
            else:
                self.cellStress = self.mpParser.cellStress_e
                self.volume = self.mpParser.volume_e
                self.spaceGroup = self.mpParser.spaceGroup
                self.pointGroup = self.mpParser.pointGroup
        else:
